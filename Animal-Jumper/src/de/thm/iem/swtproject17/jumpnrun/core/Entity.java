 ...
 * später ein Teil der Spielwelt werden ( z.B. Tiere, Hindernisse und Items die aufsammelbar sind )
 */
...
abstract class Entity { 
	...
	public void despawn() { //wird aufgerufen um eine Entität zu entfernen
		EntityControl.getInstance().despawn( this ); //entfernt die Entität aus der EntityControl
	}
	
	public void setDespawned() { //wird von der EntityComtrol aufgerufen um zu speichern ob ein Element bereits gelöscht ist oder noch nicht
		despawned = true; 
	}
	
	public boolean getDespawned() { //gibt zurück ob ein Entity bereits gelöscht ist oder noch nicht
		return despawned;
	}
	...
	abstract GameVector getSize(); //gibt die Größe der Hitbox zurück
	
	public boolean hasLogic() { //gibt zurück ob Entity Logik besitzt welche regelmäßig ausgeführt werden muss oder nicht
	...
	public boolean collidesWith( Entity entity2  ) { //gibt zurück ob sich diese Entity mit entity2 überschneidet
		GameVector dist = position.subc( entity2.getPosition() ).abs(); //berechnet die Distanz der Mittelpunkte der Entities
		PhysicalBorder border2 = entity2.getHitbox(); //gibt die Form der hitbox des 2 Elements zurück
		...
		if ( getHitbox() == PhysicalBorder.HITBOX ) { //wenn die Form der Hitbox der 1. Entity ein Rechteck ist...
			if ( border2 == PhysicalBorder.HITBOX ) { //...und dass der zweiten auch ein Rechteck ist
				...
				//kleiner als die Größe der Rechtecke ist
			...
			//im Unterschied zur ersten Berechnung wird hier die Euklidsche Distanz verwendet was zu der unterschiedlichen Form führt
		}
		else {
			entity1 = entity2;
			entity2 = this;
		...
		//Möglichst nahe an der Ellipse liegt
